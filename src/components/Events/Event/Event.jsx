...
const Event = () => {
  ...
  useEffect(() => { dispatch(getAllEvents()); }, [dispatch]);

  return (
    <Box display="flex" flexWrap="wrap">
      {events.map((event) => (
        <Link to={`/${event.id}`} key={event.id}>
          <Card width='300px' height='350px' p={2} backgroundImage={`linear-gradient(transparent, rgba(0,0,0,0.8)), url(${eventotech})`} m={2}>
            <CardHeader>
              <Flex spacing='2' alignItems='center'><Avatar size='md' name='Segun Adebayo' /></Flex>
            </CardHeader>
            <CardBody><Text fontSize='xs' color="white">{event.description}</Text></CardBody>
